...
- **"What's the actual data they need access to?"**
  - Be specific about schemas, projections, relationships
...
1. **Grep for similar functionality:**
   ```
   # Search for related schemas, contexts, components
   pattern: "[keyword from request]"
   ```

2. **Glob for relevant files:**
   ```
   # Find related views, components, domain modules
   pattern: "**/*[keyword]*"
   ```

3. **Read key files:**
   - Schemas/projections that contain the data they need
   - Contexts that handle similar workflows
   - LiveView components that could be extended
   - Domain modules that encapsulate similar logic
...
If the solution requires code (not process/config change), generate a **draft specification** ready for the `/refine-specification` command:
...
### Data Requirements
**Schemas/projections involved:**
- [Schema 1]: [what data/fields]
- [Schema 2]: [what data/fields]
...
- [Components needed: contexts, aggregates, projectors, controllers, etc.]
...
**Next command:**
```bash
/refine-specification docs/features/feature-name/specification.md
```
```
...
- Flag risks clearly: "This approach could cause performance issues if..."
